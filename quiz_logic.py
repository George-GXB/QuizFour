...
def reset_db(db_path: Path) -> None:
    """Recreate the DB schema from scratch.

    Previously this function deleted rows but left table definitions intact.
    If the schema has changed (for example the old `questions` table had a
    `category` NOT NULL column), inserting new rows without that column
    would fail. To avoid NOT NULL / schema mismatch errors, drop existing
    tables and recreate the schema by calling `init_db`.
    """
    # Drop known tables if they exist, then recreate schema
    with sqlite3.connect(db_path) as conn:
        conn.execute("DROP TABLE IF EXISTS difficulty_tags")
        conn.execute("DROP TABLE IF EXISTS default_tags")
        conn.execute("DROP TABLE IF EXISTS answer_history")
...
def export_correct_answers_to_csvs(input_dir: Path, db_path: Path) -> int:
    ...
    with sqlite3.connect(db_path) as conn:
        rows = conn.execute(
            "SELECT source_csv, row_index, answer FROM questions ORDER BY source_csv, row_index"
        ).fetchall()

    # source_csv → {row_index: correct_index}
    updates_by_csv: dict[str, dict[int, int]] = {}
    for source_csv, row_index, answer_index in rows:
        updates_by_csv.setdefault(source_csv, {})[int(row_index)] = int(answer_index)
...
def sync_csvs_to_db(input_dir: Path, db_path: Path) -> int:
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        for csv_path in csv_paths:
            ...
            questions, default_tags, difficulties = normalize_questions(df)
            ...
            for i, diff in enumerate(difficulties):
                if diff > 0:
                    conn.execute(
                        "INSERT INTO difficulty_tags (question_id, difficulty) VALUES (?, ?)",
                        (first_id + i, diff),
                    )
...
def load_difficulty_tags(db_path: Path) -> dict[int, int]:
    """Return {question_id: difficulty} for all questions with difficulty from CSV."""
    init_db(db_path)
    with sqlite3.connect(db_path) as conn:
        rows = conn.execute("SELECT question_id, difficulty FROM difficulty_tags").fetchall()
...
def normalize_questions(df: pd.DataFrame) -> tuple[list[Question], list[str], list[int]]:
    """CSVからQuestion一覧とDefaultTagリストと難易度リストを返す。

    Returns: (questions, default_tags, difficulties)
    """
    ...
    for idx, row in df.iterrows():
        ...
        if has_difficulty:
            try:
                diff_val = int(row["difficulty"])
                difficulties.append(max(1, min(5, diff_val)))
            except (TypeError, ValueError):
                difficulties.append(0)
        else:
            difficulties.append(0)
    return questions, default_tags, difficulties
